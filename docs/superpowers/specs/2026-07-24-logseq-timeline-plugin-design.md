...
1. **Query layer** — one `logseq.DB.datascriptQuery` call fetching all `#tl`-tagged nodes: uuid, title, `tl-date` (nested pull → value-entity title), `tl-topic` (nested pull → titles + uuids), `tl-type` (nested pull → title). Property idents are discovered at runtime by property title (`tl-date` etc.), since user-property idents may be UUID-suffixed. **All three properties are ref-typed** — live-graph verification (2026-07-24) showed that in DB graphs even Text (`:default`) property values are refs to hidden value-entities whose `:block/title` carries the text; nested pull specs resolve all of them inline.
